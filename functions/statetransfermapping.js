			...
			{	
				...
				if(ajaxresponse[0] == '1')
				{
					...
					if(ajaxresponse[3] > '0')
					{
						...
						divenable('statename','form_dlrlist');
						divenable('categoryname','form_dlrlist');
						divenable('dlrtransfer','form_dlrlist');
						$(".statenamedlr").html('');
						$(".statenamedlr").html('Select state name below');
						$(".categorystate").html('');
						$(".categorystate").html('Select state name first');
...
{
	var passdata = "&submittype=category&form_dlrlist=" + encodeURIComponent($("#form_dlrlist").val())+"&form_state=" + encodeURIComponent($("#form_state").val())+"&dummy=" + Math.floor(Math.random()*100032680100);
	//alert(passdata);
	var outputselect = $("#form_category");
	var queryString = "../ajax/statetransfermapping.php";
	ajaxobjext38 = $.ajax(
	{
		type: "POST",url: queryString, data: passdata, cache: false,
		...
		{	
			$(".categorystate").html('');
			$(".categorystate").html('Select category name below');
...
{
	$("#gridprocess").html(processing());
	var passdata = "&submittype=gridcategory&form_dlrlist=" + encodeURIComponent($("#form_dlrlist").val())+"&form_state=" + encodeURIComponent($("#form_state").val())+"&form_category=" + encodeURIComponent($("#form_category").val())+"&dummy=" + Math.floor(Math.random()*100032680100);
	//alert (passdata);
	var queryString = "../ajax/statetransfermapping.php";
	ajaxobjext31 = $.ajax(
	{
...
function transferdata()
{
	...
	{ 
		msg_box.html(errormessage("Sorry!! Kindly Select 'All OR Category' From 'Select a category'.")); 
		state.focus(); 
		return false;
	}
	else if (name.val() == '')
	...
	var passdata = "&submittype=transferdata&form_dlrlist1=" + encodeURIComponent($("#form_dlrlist1").val())+"&form_state=" + encodeURIComponent($("#form_state").val())+"&form_dlrlist=" + encodeURIComponent($("#form_dlrlist").val())+"&form_category=" + encodeURIComponent($("#form_category").val())+"&dummy=" + Math.floor(Math.random()*100032680100);
	//alert (passdata);
	var queryString = "../ajax/statetransfermapping.php";
	ajaxobjext31 = $.ajax(
	{
