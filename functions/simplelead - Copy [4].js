...
function griddata(startlimit)
{
	...
	ajaxobjext55 = $.ajax(
	{
		...
		success: function(response,status)
		{
			...
			{	
				var response1 = response.split("|^|"); //alert(response1);
				var leadvalues = response1[4].split('$')
				for( var i=0; i<leadvalues.length; i++)
				{
					leadarray[i] = leadvalues[i];
				}
				if(response1[0] == '1')
				{
					...
					$("#hiddenleadcount").val(response1[3]);
				}
...
function filtering(command)
{
	...
	if($("#considerfollowup:checked").val() == 'on')
	{
		...
		if($("#entereddatecheck").is(":checked") == true)
		{
			var datecheck = 'entereddatecheck';
		}	
		else if($("#nextfollowupdatecheck").is(":checked") == true)
		{
			var datecheck = 'nextfollowupdatecheck';
		}
	}
	else
	{
	...
	if($("#onlydemogiven:checked").val() == 'true')
	{
		var onlydemogiven = 'true';
	}
	else
	{
		var onlydemogiven = 'false';
	}
	if(command == 'excel')
		 $("#filterform").submit();
		...
		ajaxobjext56 = $.ajax(
		{
			...
			{	
				...
				else
				{
					...
					var leadvalues = response2[4].split('$'); 
					for( var i=0; i<leadvalues.length; i++)
					{
						leadarray[i] = leadvalues[i];
					}
					if(response2[0] == '1')
					{
						...
						msg_box.html('');
						$('#hiddenleadcount').val(response2[3]);
					}
		...
		success: function(response,status)
		{
			...
			{	
				...
				if(response3[0] == '1')
				{
					...
					$('#hiddenid').val(response3[3]); 
				//	alert(leadarray);
					leadarrayvalue = $.inArray(response3[3], leadarray);
					$('#displayrowcount').html((leadarrayvalue+1) + ' of ' + $('#hiddenleadcount').val());
					$('#hiddentype').val(leadarrayvalue);
...
function getnextorpreviousvalue(buttontype)
{
	arrayindexvalue= $('#hiddentype').val();

	if(buttontype == 'previous')
	{
		if(arrayindexvalue == 0)
		{
			arrayindex = (arrayindexvalue*1)
			hiddenleadid = leadarray[arrayindex];
		}
		else
		{
			arrayindex = (arrayindexvalue*1) - 1
			hiddenleadid = leadarray[arrayindex];
			gridtoform(hiddenleadid);
		}
	}
	else if(buttontype == 'next')
	{
		if(arrayindexvalue == (leadarray.length)-1)
		{
			arrayindex = (arrayindexvalue*1)
			hiddenleadid = leadarray[arrayindex];
		}
	...
	{
		arrayindex = 0;
		hiddenleadid = leadarray[0];	
		gridtoform(hiddenleadid);
	}
	else if(buttontype == 'last')
	{
		arrayindex = (leadarray.length)-1;
		hiddenleadid = leadarray[arrayindex];
		gridtoform(hiddenleadid);
	}
	$('#hiddentype').val(arrayindex)
	//alert(arrayindex + hiddenleadid)
	//alert(hiddenleadid);
...
function formsubmit(command)
{
	...
	if(confirmation)
	{
		...
		var passdata = "&submittype=save&form_recid=" + form_recid.val() + "&form_leadstatus=" + encodeURIComponent(form_leadstatus.val())+"&dummy=" + Math.floor(Math.random()*10230000000);//alert(passdata)
		var queryString = "../ajax/simplelead.php";
		ajaxobjext60 = $.ajax(
		{
			type: "POST",url: queryString, data: passdata, cache: false,
...
function filterfollowupdates()
{
	...
	var followupmade = $("#followupmade");
	var followuppending = $("#followuppending");
	var entereddatecheck = $("#entereddatecheck");
	var nextfollowupdatecheck = $("#nextfollowupdatecheck");
	if($("#considerfollowup:checked").val() != 'on')
	{
		...
		entereddatecheck.attr('disabled',true);
		nextfollowupdatecheck.attr('disabled',true);
	}
	else if($("#considerfollowup:checked").val() == 'on')
	{
		...
		entereddatecheck.attr('disabled',false);
		nextfollowupdatecheck.attr('disabled',false);
	}
}
		...
		ajaxobjext57 = $.ajax(
		{
			...
			success: function(response,status)
			{
				...
				{	
					...
					var leadvalues = ajaxresponse[4].split('$')
					for( var i=0; i<leadvalues.length; i++)
					{
						leadarray[i] = leadvalues[i];
					}
			...
			success: function(response,status)
			{
				...
				{	
					...
					if(ajaxresponse[0] == '1')
					{
						...
						$('#hiddenleadcount').val(ajaxresponse[3]);
					}
					else if(ajaxresponse[0] == '2')
					{
						$('#resultgridf1').html( $('#tabgroupgridf1_1').html());
...
function gridtoexcel(command)
{
  $('#toexcelform').attr("action", "../reportslms/uploadleadtoexcel.php?command="+command) ;
  $('#toexcelform').attr( 'target', '_blank' );
  $('#toexcelform').submit();	
}