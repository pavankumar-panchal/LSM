...
switch($submittype)
{
	...
	case "save":
		...
		if($message == "")
		{
			$query = "SELECT * FROM leads WHERE (emailid = '".$form_email."' || cell = '".$form_cell."') AND productid = '".$form_product."' and  leaddatetime > DATE_ADD(CURDATE(),INTERVAL -365  DAY)";
			...
			if($count > 0)
			{
				...
				$result = runmysqlqueryfetch($query);
				$leadid = $result['id'];
				$message = "2^This email ID/Cell No is already available as a lead to same product [Lead ID: ".$leadid."]. You can upload 
				it for someother product.";
				//
			}
		}
		...
		if($message == "")
		{
			if($form_product == "30")
			{
			    $query = "SELECT * FROM dealers WHERE id = 1000154 ";
				$result = runmysqlqueryfetch($query);
				$dealerid = $result['id'];	
			}
			else
			{
				if($form_dealer == 'mapping')
				{
					...
					$query1 = "SELECT *,leads.dealerid AS dealerid FROM leads left join lms_users on leads.dealerid = lms_users.referenceid WHERE (leads.emailid = '".$form_email."' || leads.cell = '".$form_cell."') AND leads.dealerid <> '999999' AND lms_users.disablelogin <> 'yes';";
					...
					else
					{
						...
						$query = "SELECT * FROM mapping left join lms_users on mapping.dealerid = lms_users.referenceid WHERE mapping.regionid = '".$form_region."' AND mapping.prdcategory = '".$prdcategory."' 
						AND lms_users.disablelogin <>'yes';";
						$result = runmysqlquery($query);
			...
			$query2 = "insert into `leads` (dealerid, productid, source, company, name, address, place, regionid, phone, emailid, refer, leaduploadedby, leadstatus, 
			leadremarks, dealernativeid, cell,stdcode,initialcontactname,initialaddress,initialstdcode,initialphone,initialcellnumber,initialemailid,leaddatetime,
			dlrmbrid,website)values('".$dealerid."', '".$form_product."','Manual Upload', '".$form_companyname."', '".$form_name."', '".$form_address."', '".$form_place."', 
			'".$form_region."', '".$form_phone."', '".$form_email."', '".$form_source."', '".$enteredbyuserid."', 'Not Viewed',
			'".$form_leadremarks."', '".$dealerid."', '".$form_cell."', '".$form_stdcode."' ,'".$form_name."','".$form_address."',
			'".$form_stdcode."','".$form_phone."','".$form_cell."','".$form_email."','".$leaddate.' '.$leadtime."','".$dlrmbrid."','".$form_website."')";
		...
		case 'dealertooltip':
		{
			...
			$query5 = "select dlrcompanyname,dlrname,district,state,dlrcell,dlrphone,dlremail from dealers 
			where id = '".$dealerid."'";
