...
switch($submittype)
{
	case "thisfinancialyear":
		...
		where leads.leaddatetime between '".$fybegin."' and '".$fyend."' ".$sourcepiece." ".$areapiece." ".$productgrouppiece." group by months order by leads.leaddatetime";
		//echo($query); exit;
	...
	case "lastfinancialyear" :
		...
		left join products on products.id = leads.productid 
		left join regions on leads.regionid = regions.subdistcode  
		where leads.leaddatetime between '".$fybegin."' and '".$fyend."' ".$sourcepiece." ".$areapiece." ".$productgrouppiece." group by months order by months";
		//echo($query); exit;
		$result = runmysqlquery($query);
		$monthsarray = array();
		$graphdata =  array();
	...
	case "thismonth":
		...
		$query = "select count(*) as counts,left(leads.leaddatetime,10) as dates from leads left join dealers on leads.dealerid = dealers.id  left join lms_managers on lms_managers.id = dealers.managerid left join products on products.id = leads.productid  left join regions on leads.regionid = regions.subdistcode where month(leads.leaddatetime) = (month(curdate())) and year(leads.leaddatetime) = (year(curdate())) ".$sourcepiece." ".$areapiece." ".$productgrouppiece."  group by left(leads.leaddatetime,10)";
		$result = runmysqlquery($query);
	...
	//	echo($year);exit;
		...
		$query = "select count(*) as counts,left(leads.leaddatetime,10) as dates from leads left join dealers on leads.dealerid = dealers.id  left join lms_managers on lms_managers.id = dealers.managerid left join products on products.id = leads.productid  left join regions on leads.regionid = regions.subdistcode where month(leads.leaddatetime) = ".$monthpiece ." and year(leads.leaddatetime) = ".$yearpiece." ".$sourcepiece." ".$areapiece." ".$productgrouppiece." group by left(leads.leaddatetime,10)";
		$result = runmysqlquery($query);
		$datearray =  array();
		$completedate = array();
		$graphdata =  array();
