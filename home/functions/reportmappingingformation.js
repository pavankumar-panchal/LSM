...
function filter(command)
{
	...
	else
	{
		...
		var generate = $("input[name='generate']:checked").val();
		...
		var passdata = "&submittype=filter&searchtext="+searchtext+"&category="+category+"&state="+state+"&district="+district+"&region="+region+"&disabled="+disabled+"&radiovalue="+radiovalue+"&orderby="+orderby+"&generate="+generate+"&dummy=" + Math.floor(Math.random()*100032680100); //alert(passdata);
		var queryString = "../ajax/reportmappinginformation.php"; 
		ajaxobjext101 = $.ajax(
		{
			type: "POST",url: queryString, data: passdata, cache: false,
