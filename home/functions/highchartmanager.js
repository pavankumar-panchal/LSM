function retrievedata()
{
	...
	var passdata = "&submittype="+type+"&source="+source+"&area="+area+"&productgroup="+productgroup+"&dummy=" + Math.floor(Math.random()*100032680100);
	var queryString = "../ajax/graphdatamanager.php";
	ajaxobjext48 = $.ajax(
	{
		...
		success: function(response,status)
		{
			...
			{	
				if(type == 'thismonth'  || type == 'lastmonth')
				{
					...
					$('#alldates').val(splitval[0]); //alert(document.getElementById('alldates').value)
					displaygraph(type);
				}
				else
				{
