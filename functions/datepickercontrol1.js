...
DatePickerControl.editIdPrefix    = "DPC1_";          // The prefix for edit's id
 ...
 * attibute datepicker="true" or id="DPC1_foo_[format]"
 */
DatePickerControl.init = function()
{
	...
	if (!document.getElementById("CalendarPickerControl")){
		...
		if (this.calContainer.addEventListener){
			this.calContainer.addEventListener("click", DPC1_onContainerClick, false);
			window.addEventListener("resize", DPC1_onWindowResize, false);
		}
		else if (this.calContainer.attachEvent){
			this.calContainer.attachEvent("onclick", DPC1_onContainerClick);
			window.attachEvent("onresize", DPC1_onWindowResize);
		}
	}
		...
		if (inputControls[i].type.toLowerCase() == "text"){
			...
			if (setEvents){
				if(editctrl.addEventListener){
					editctrl.addEventListener("keyup", DPC1_onEditControlKeyUp, false);
					editctrl.addEventListener("keydown", DPC1_onEditControlKeyDown, false);
					editctrl.addEventListener("keypress", DPC1_onEditControlKeyPress, false);
					editctrl.addEventListener("blur", DPC1_onEditControlBlur, false);
					editctrl.addEventListener("focus", DPC1_onEditControlFocus, false);
					editctrl.addEventListener("change", DPC1_onEditControlChange, false);
				}
				else if (editctrl.attachEvent){
					editctrl.attachEvent("onkeyup", DPC1_onEditControlKeyUp);
					editctrl.attachEvent("onkeydown", DPC1_onEditControlKeyDown);
					editctrl.attachEvent("onkeypress", DPC1_onEditControlKeyPress);
					editctrl.attachEvent("onblur", DPC1_onEditControlBlur);
					editctrl.attachEvent("onfocus", DPC1_onEditControlFocus);
					editctrl.attachEvent("onchange", DPC1_onEditControlChange);
				}
				var theForm = editctrl.form;
				if (!this.onSubmitAsigned && theForm){
					this.onSubmitAsigned = true;
					theForm.submitOrig = theForm.submit;
					theForm.submit = DPC1_formSubmit;
					if (theForm.addEventListener){
						theForm.addEventListener('submit', DPC1_onFormSubmit, false);
					}
					else if (theForm.attachEvent){
						theForm.attachEvent('onsubmit', DPC1_onFormSubmit);
					}
				}
			}
		}
...
{
	var obj = document.getElementById("DPC1_DEFAULT_FORMAT");
	if (obj) this.defaultFormat = obj.value;

	obj = document.getElementById("DPC1_SUBMIT_FORMAT");
	if (obj) this.submitFormat = obj.value;
	
	obj = document.getElementById("DPC1_FIRST_WEEK_DAY");
	if (obj) this.firstWeekDay = (obj.value < 0 || obj.value > 6) ? 0 : parseInt(obj.value);
	
	obj = document.getElementById("DPC1_WEEKEND_DAYS");
	if (obj) eval("this.weekend = " + obj.value);
	
	obj = document.getElementById("DPC1_AUTO_SHOW");
	if (obj) this.autoShow = obj.value == "true";
	
	obj = document.getElementById("DPC1_DEFAULT_TODAY");
	if (obj) this.defaultTodaySel = obj.value == "true";
	
	obj = document.getElementById("DPC1_CALENDAR_OFFSET_X");
	if (obj) this.offsetX = parseInt(obj.value);
	
	obj = document.getElementById("DPC1_CALENDAR_OFFSET_Y");
	if (obj) this.offsetY = parseInt(obj.value);
	
	obj = document.getElementById("DPC1_TODAY_TEXT");
	if (obj) this.todayText = obj.value;
	
	obj = document.getElementById("DPC1_BUTTON_TITLE");
	if (obj) this.buttonTitle = obj.value;
	
	obj = document.getElementById("DPC1_BUTTON_POSITION");
	if (obj) this.buttonPosition = obj.value;
	
	obj = document.getElementById("DPC1_BUTTON_OFFSET_X");
	if (obj) this.buttonOffsetX = parseInt(obj.value);
	
	obj = document.getElementById("DPC1_BUTTON_OFFSET_Y");
	if (obj) this.buttonOffsetY = parseInt(obj.value);
	
	obj = document.getElementById("DPC1_WEEK_NUMBER");
	if (obj) this.weekNumber = obj.value == "true";
	
	obj = document.getElementById("DPC1_MONTH_NAMES");
	if (obj) eval("this.Months = " + obj.value);
	
	obj = document.getElementById("DPC1_DAY_NAMES");
 ...
 * Wrapper for init()
 */
function DPC1_autoInit()
{
	DatePickerControl.init();
}

if (window.addEventListener){
	window.addEventListener("load", DPC1_autoInit, false);
}
else if (window.attachEvent){
	window.attachEvent("onload", DPC1_autoInit);
}
...
DatePickerControl.createButton = function(input, useId)
{
	...
	if(calButton.addEventListener){
		calButton.addEventListener("click", DPC1_onButtonClick, false);
	}
	else if (calButton.attachEvent){
		calButton.attachEvent("onclick", DPC1_onButtonClick);
	}
 ...
 */
function DPC1_onButtonClick(event){DatePickerControl.onButtonClick(event);}
 ...
 */
function DPC1_onContainerClick(event){DatePickerControl.onContainerClick(event);}
 ...
 */
function DPC1_onEditControlKeyUp(event){DatePickerControl.onEditControlKeyUp(event);}
 ...
 */
function DPC1_onEditControlKeyDown(event){DatePickerControl.onEditControlKeyDown(event);}
 ...
 */
function DPC1_onEditControlKeyPress(event){DatePickerControl.onEditControlKeyPress(event);}
 ...
 */
function DPC1_onEditControlBlur(event){DatePickerControl.onEditControlBlur(event);}
 ...
 */
function DPC1_onEditControlChange(event){DatePickerControl.onEditControlChange(event);}
 ...
 */
function DPC1_onEditControlFocus(event){DatePickerControl.onEditControlFocus(event);}
 ...
 * Form's submit event
 */
function DPC1_onFormSubmit(event){DatePickerControl.onFormSubmit(event);}
 ...
 * Window resize event.
 */
function DPC1_onWindowResize(event){DatePickerControl.onWindowResize(event);}
