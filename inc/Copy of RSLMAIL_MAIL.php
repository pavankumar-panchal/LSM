...
function rslmail($fromname, $fromemail, $toarray, $subject, $text, $html, $ccarray = null, $bccarray = null, $filearray = null)
{
	$smtp = 'mail.relyon.co.in';
	$username = 'lms@relyon.co.in';
	$password = 'itzzzlms';
	$smtp_port = 25;
	$usessl = false;
	$timeout = 30;
...
class RSLMAIL_FUNC
{
	...
	static public function is_alpha($strval, $numeric = true, $addstr = '')
	{
		try {
			...
			if (!is_bool($numeric))
				$errors[] = 'invalid numeric type';
			if (!is_string($addstr))
				$errors[] = 'invalid additional type';
			if (count($errors) == 0) {
				if ($strval != '') {
			...
			if (count($errors) == 0) {
				...
				if (trim($str) != '' && self::is_alpha($str, true, '-.')) {
					if (count($exphost1 = explode('.', $str)) > 1 && !(strstr($str, '.-') || strstr($str, '-.'))) {
						...
						$len = strlen($ext);
						if ($set1 && $set2 && $len > 1 && $len < 7 && self::is_alpha($ext, false))
							$ret = true;
					}
				}
	...
	static public function is_ipv4($str)
	{
		try {
			if (is_string($str))
				return (trim($str) != '' && ip2long($str) && count(explode('.', $str)) === 4);
			else
				throw new Exception('invalid argument value', 0);
		} catch (Exception $e) {
			return self::exception_handler($e);
		}
	}
	...
	static public function getmxrr_win($hostname, &$mxhosts)
	{
		...
		try {
			if (is_string($hostname)) {
				...
				throw new Exception('invalid argument type', 0);
		} catch (Exception $e) {
			return self::exception_handler($e);
		}
	}
			...
			if (is_string($filename)) {
				...
				if ($filename != '') {
					if (count($exp = explode('.', $filename)) >= 2) {
						...
						if (isset(self::$_extensions[$ext]))
							$ret = self::$_extensions[$ext];
					}
					return $ret;
				} else
					throw new Exception('invalid argument value', 0);
...
class RSLMAIL_MIME extends RSLMAIL_FUNC
{
	...
	static public function qp_encode($str = null, $len = self::MLEN, $end = self::CRLF)
	{
		try {
			...
			if (!(is_string($str) && $str != ''))
				$errors[] = 'invalid argument type';
			if (!(is_int($len) && $len > 1))
				$errors[] = 'invalid line length value';
			if (!is_string($end))
				$errors[] = 'invalid line end value';
			if (count($errors) == 0) {
				$out = '';
	...
	static public function encode_header($str = null, $charset = self::HCHARSET, $encoding = self::HENCODING)
	{
		try {
			...
			if (!(is_string($encoding) && isset(self::$_hencoding[$encoding])))
			...
			if (count($errors) == 0) {
				...
				if ($encoding == 'quoted-printable') {
					...
					$enc = rtrim(chunk_split(base64_encode($str), self::HLEN, self::CRLF));
				if ($enc) {
					$res = array();
					$chr = ($encoding == 'base64') ? 'B' : 'Q';
					foreach (explode(self::CRLF, $enc) as $val)
						if ($val != '')
							$res[] = '=?' . $charset . '?' . $chr . '?' . $val . '?=';
	...
	static public function decode_content($str = null, $decoding = '7bit')
	{
		try {
			...
			if (!is_string($decoding))
				$errors[] = 'invalid decoding type';
			else {
				$decoding = self::str_clear($decoding, array(' '));
				$decoding = strtolower($decoding);
				if (!isset(self::$_mencoding[$decoding]))
					$errors[] = 'invalid decoding value';
			}
			if (count($errors) == 0) {
				if ($decoding == 'base64') {
	...
	static public function message($content = null, $type = null, $name = null, $charset = null, $encoding = null, $disposition = null, $id = null, $exception = null)
	{
		try {
			...
			if ($name == null)
				...
				if ($name != '') {
					...
					if ($namelen < 2 || $namelen > 64)
						$errors[] = 'invalid name value';
				}
			} else
				$errors[] = 'invalid name type';
			if ($charset == null)
				$charset = '';
			...
			if ($encoding == null)
				...
				if (!isset(self::$_mencoding[$encoding]))
					$errors[] = 'invalid encoding value';
			} else
				$errors[] = 'invalid encoding type';
			if ($disposition == null)
				$disposition = 'inline';
				...
				if (!($disposition == 'inline' || $disposition == 'attachment'))
					$errors[] = 'invalid disposition value';
			} else
				$errors[] = 'invalid disposition type';
			if ($id == null)
				$id = '';
			...
			if (count($errors) == 0) {
				...
				if ($encoding == '7bit' || $encoding == '8bit')
					$content = wordwrap($content, self::MLEN, self::CRLF, true);
				else if ($encoding == 'base64')
					$content = chunk_split(base64_encode($content), self::MLEN, self::CRLF);
				else if ($encoding == 'quoted-printable')
					$content = self::qp_encode($content);
				return array('name' => $name, 'disposition' => $disposition, 'header' => $header, 'content' => $content);
			} else
				throw self::exception_rewrite($exception, implode(', ', $errors), 0);
		} catch (Exception $e) {
			return self::exception_handler($e, false);
		}
	}
	...
	static public function compose($text = null, $html = null, $attach = null, $uniq = null)
	{
		try {
			...
			if ($text == null && $html == null)
				...
				if ($text != null) {
					if (!(is_array($text) && isset($text['header'], $text['content']) && is_string($text['header']) && is_string($text['content'])))
						$errors[] = 'invalid text message format';
				}
				if ($html != null) {
					if (!(is_array($html) && isset($html['header'], $html['content']) && is_string($html['header']) && is_string($html['content'])))
						$errors[] = 'invalid html message format';
				}
				if ($attach != null) {
				...
				if ($multipart) {
					...
					if ($text && $html) {
						if ($disp['inline'] && $disp['attachment']) {
							...
							foreach ($attach as $desc)
								if ($desc['disposition'] == 'inline')
									$body .= '--' . $boundary2 . self::CRLF . $desc['header'] . self::CRLF . self::CRLF . $desc['content'] . self::CRLF;
							$body .= '--' . $boundary2 . '--' . self::CRLF; foreach ($attach as $desc)
								if ($desc['disposition'] == 'attachment')
									$body .= '--' . $boundary1 . self::CRLF . $desc['header'] . self::CRLF . self::CRLF . $desc['content'] . self::CRLF;
							$body .= '--' . $boundary1 . '--';
						} else if ($disp['inline']) {
	...
	static public function split_message($body = null, $multipart = null, $boundary = null)
	{
		try {
			$errors = array();
			if (!is_string($body))
				$errors[] = 'invalid body type';
			if (!is_string($multipart))
				$errors[] = 'invalid multipart type';
			if (!is_string($boundary))
				$errors[] = 'invalid boundary type';
			if (count($errors) == 0) {
				$ret = array();
							...
							if ($data1 = stristr($part, 'content-type: ')) {
								if ($data2 = stristr($part, 'boundary=')) {
									...
									if ($multipart2 == 'mixed' || $multipart2 == 'related' || $multipart2 == 'alternative') {
										...
										if ($boundary2 != '')
											$ret = self::split_message($part, $multipart . ', ' . $multipart2, $boundary2);
									}
								} else {
									if ($res = self::split_compose($part)) {
										$one = array();
										foreach ($res['header'] as $harr) {
											foreach ($harr as $hnum => $hval)
												if (stristr($hnum, 'content-'))
													$one[$hnum] = $hval;
										}
	...
	static public function split_compose($str = null)
	{
		try {
			...
			if (!(count($exp1 = explode($sep, $str)) > 1)) {
				...
				$sep = $new . $new;
				if (!(count($exp1 = explode($sep, $str)) > 1))
					throw new Exception('invalid 1 argument value', 1);
			}
			$multipart = false;
			$header = str_replace(';' . $new . "\t", '; ', $exp1[0]);
			$header = str_replace($new . "\t", '', $header);
			if (!(count($exp2 = explode($new, $header)) > 1))
				throw new Exception('invalid 2 argument value', 1);
					...
					if (strtolower($name) == 'content-type') {
						if (($data1 = stristr($sval, 'multipart/')) && ($data2 = stristr($sval, 'boundary='))) {
							...
							if ($boundary != '') {
								...
								if ($mtype == 'mixed' || $mtype == 'related' || $mtype == 'alternative')
									$multipart = $mtype;
							}
						}
					}
			...
			if (count($arr['header']) > 0) {
				...
				throw new Exception('invalid 3 argument value', 1);
		} catch (Exception $e) {
			return self::exception_handler($e, false);
		}
	}
	...
	static public function split_content($str = null)
	{
		try {
			if (!is_string($str))
				throw new Exception('invalid argument type', 0);
			if (!$res = self::split_compose($str))
				throw new Exception('invalid 1 argument value', 1);
			$arr = array();
			if (isset($res['multipart'], $res['boundary'])) {
				$arr['header'] = $res['header'];
				$arr['multipart'] = 'yes';
				if (!$arr['body'] = self::split_message($res['body'], $res['multipart'], $res['boundary']))
					throw new Exception('invalid 2 argument value', 1);
			} else {
				foreach ($res['header'] as $harr) {
					foreach ($harr as $hnum => $hval)
...
class RSLMAIL_SMTP extends RSLMAIL_MIME
{
	...
	static public function quit($fp)
	{
		...
		if ($fp && is_resource($fp)) {
			if (fwrite($fp, 'QUIT' . self::CRLF)) {
				if ($vget = @fgets($fp, 1024))
					$res['success'] = $vget;
				$ret = true;
			} else
				$res[27] = 'can not write';
			@fclose($fp);
		} else
			$res[26] = 'invalid resource connection';
	...
	static public function connect($host = null, $user = null, $pass = null, $port = null, $ssl = null, $timeout = null, $name = null, $exception = null)
	{
		try {
			...
			if ($host == null)
				...
				$errors[] = 'invalid hostname type';
			if ($user == null)
				$user = '';
			else if (!is_string($user))
				$errors[] = 'invalid username type';
			if ($pass == null)
				$pass = '';
			else if (!is_string($pass))
				$errors[] = 'invalid password type';
			if ($port == null)
				$port = self::PORT;
			else if (!(is_int($port) && $port > 0))
				$errors[] = 'invalid port type/value';
			if ($ssl == null)
				$ssl = false;
			else if (is_string($ssl) && isset(self::$_cssl[strtolower($ssl)]))
				$ssl = strtolower($ssl);
			else if (!is_bool($ssl))
				$errors[] = 'invalid ssl value';
			if ($timeout == null)
				$timeout = self::TIMEOUT;
			else if (!(is_int($timeout) && $timeout > 0))
				$errors[] = 'invalid timeout type/value';
			if ($name == null)
				$name = '';
			else if (!is_string($name))
				$errors[] = 'invalid name type';
			if (count($errors) == 0) {
				...
				if ($host == '')
					$errors[] = 'invalid hostname value';
				else if (self::is_ipv4($host))
					$iphost = $host;
				else {
					$iphost = gethostbyname($host);
					if ($iphost == $host)
						$errors[] = 'invalid hostname address';
				}
				$user = self::str_clear($user);
				$pass = self::str_clear($pass);
				if (($user != '' && $pass == '') || ($user == '' && $pass != ''))
					$errors[] = 'invalid username and password value combination';
				if (count($errors) == 0) {
					...
					if ($name == '')
						$name = '127.0.0.1';
					if (!$fp = stream_socket_client($ver . '://' . $iphost . ':' . $port, $errno, $errstr, $timeout))
						$arr['result'][0] = $errstr;
					else if (!stream_set_timeout($fp, self::COM_TIMEOUT))
						$arr['result'][1] = 'could not set stream timeout';
					else if (!self::result($fp, $response, 220))
						$arr['result'][2] = $response;
					else if (!fwrite($fp, 'EHLO ' . $name . self::CRLF))
						$arr['result'][3] = 'can not write';
					else {
						$continue = false;
						if (!self::result($fp, $response, 250)) {
							if (!fwrite($fp, 'HELO ' . $name . self::CRLF))
								$arr['result'][4] = 'can not write';
							else if (!self::result($fp, $response, 250))
								$arr['result'][5] = $response;
							else
								$continue = true;
						} else
							$continue = true;
						if ($continue) {
							if ($user != '') {
								if (!fwrite($fp, 'AUTH LOGIN' . self::CRLF))
									$arr['result'][6] = 'can not write';
								else if (!self::result($fp, $response, 334)) {
									if (!fwrite($fp, 'AUTH PLAIN ' . base64_encode($user . chr(0) . $user . chr(0) . $pass) . self::CRLF))
										$arr['result'][7] = 'can not write';
									else if (!self::result($fp, $response, 235))
										$arr['result'][8] = $response;
									else
										$arr['connection'] = $fp;
								} else if (!fwrite($fp, base64_encode($user) . self::CRLF))
									$arr['result'][9] = 'can not write';
								else if (!self::result($fp, $response, 334))
									$arr['result'][10] = $response;
								else if (!fwrite($fp, base64_encode($pass) . self::CRLF))
									$arr['result'][11] = 'can not write';
								else if (!self::result($fp, $response, 235))
									$arr['result'][12] = $response;
								else
									$arr['connection'] = $fp;
							} else
								$arr['connection'] = $fp;
	...
	static public function sendtohost($host, $addrs, $from, $message, $name = '', $path = '', $port = self::PORT, $timeout = self::TIMEOUT, $exception = null)
	{
		try {
			$errors = array();
			if (!(is_string($host) || is_resource($host)))
				$errors[] = 'invalid ip/connection type';
			if (!is_array($addrs))
				$errors[] = 'invalid mail address destination type';
			else {
				if (count($addrs) > 0) {
			...
			if (!is_string($from))
				$errors[] = 'invalid mail from address type';
			if (!is_string($path))
					...
					if (self::is_ipv4($host))
						...
						if ($havemx) {
							foreach ($mx as $mxhost) {
								...
								if ($iphost != $mxhost && !isset($iparr[$iphost]) && self::is_ipv4($iphost))
									$iparr[$iphost] = $iphost;
							}
						} else {
							$iphost = gethostbyname($host);
							if ($iphost != $host && self::is_ipv4($iphost))
								$iparr[$iphost] = $iphost;
						}
					}
					if (count($iparr) > 0) {
						foreach ($iparr as $ipaddr) {
							...
							if ($fp = $conn['connection'])
								break;
							else
								$res = $conn['result'];
						}
					} else
						$res['error'] = 'can not find any valid ip address';
				} else {
					$quit = false;
					$fp = $host;
				}
				if ($fp) {
					if (!fwrite($fp, 'MAIL FROM:<' . (($path == '') ? $from : $path) . '>' . self::CRLF))
						$res[13] = 'can not write';
					else if (!self::result($fp, $response, 250))
						$res[14] = $response;
					else {
						$continue = true;
						...
						if ($continue) {
							if (!fwrite($fp, 'DATA' . self::CRLF))
								$res[17] = 'can not write';
							else if (!self::result($fp, $response, 354))
								$res[18] = $response;
							else {
								...
								if ($continue) {
									if (!fwrite($fp, '.' . self::CRLF))
										$res[20] = 'can not write';
									else if (!self::result($fp, $response, 250))
										$res[21] = $response;
									else {
										if (!fwrite($fp, 'RSET' . self::CRLF))
											$res[22] = 'can not write';
										else if (!self::result($fp, $response, 250))
											$res[23] = $response;
										else if ($quit) {
											if (!fwrite($fp, 'QUIT' . self::CRLF))
												$res[24] = 'can not write';
											else if (!$vget = @fgets($fp, 1024))
												$res[25] = $response;
											else
												$res['success'] = $vget;
										} else
...
class RSLMAIL_MAIL
{
	...
	public function delivery($str = null)
	{
		try {
			...
			return RSLMAIL_FUNC::exception_handler($e);
		}
	}
	...
	private function _addaddr($addr, $name, $charset, $encoding, &$rep, $exception)
	{
		...
		try {
			...
			if (count($errors) == 0) {
				if (isset($rep[$addr]))
					throw RSLMAIL_FUNC::exception_rewrite($exception, 'address already exists', 1);
				else if (!RSLMAIL_FUNC::is_mail($addr))
					throw RSLMAIL_FUNC::exception_rewrite($exception, 'invalid address value', 1);
				else {
				...
				throw RSLMAIL_FUNC::exception_rewrite($exception, implode(', ', $errors), 0);
		} catch (Exception $e) {
			return RSLMAIL_FUNC::exception_handler($e, $ret);
		}
	}
	...
	public function addbcc($addr = null)
	{
		try {
			if (!is_string($addr))
				throw new Exception('invalid address type', 0);
			else if (!RSLMAIL_FUNC::is_mail($addr))
				throw new Exception('invalid address value', 0);
			else if (isset($this->_bcc[$addr]))
				throw new Exception('address already exists', 1);
			else {
				$this->_bcc[$addr] = false;
				return true;
			}
		} catch (Exception $e) {
			return RSLMAIL_FUNC::exception_handler($e, false);
		}
	}
	...
	private function _deladdr($addr, &$rep, $exception)
	{
		try {
			if ($addr == null) {
				...
				throw RSLMAIL_FUNC::exception_rewrite($exception, 'invalid address value', 0);
		} catch (Exception $e) {
			return RSLMAIL_FUNC::exception_handler($e);
		}
	}
	...
	public function from($addr = null, $name = null, $charset = null, $encoding = null)
	{
		...
		try {
			...
			if (count($errors) == 0) {
				...
				$name = trim($name);
				if ($name == '')
					$this->_from = array('address' => $addr, 'name' => false);
				else {
					$code = RSLMAIL_MIME::encode_header($name, $charset, $encoding);
		...
		try {
			...
			if (count($errors) == 0) {
				...
				if ($name == '')
					$errors[] = 'invalid name value';
				if ($value == '')
					$errors[] = 'value are empty';
				if (count($errors) == 0) {
					$ver = strtolower($name);
					$err = false;
					if ($ver == 'to')
						$err = 'can not set "To", for this, use function "AddTo()"';
					else if ($ver == 'cc')
						$err = 'can not set "Cc", for this, use function "AddCc()"';
					else if ($ver == 'bcc')
						$err = 'can not set "Bcc", for this, use function "AddBcc()"';
					else if ($ver == 'from')
						$err = 'can not set "From", for this, use function "From()"';
					else if ($ver == 'subject')
						$err = 'can not set "Subject", for this, use function "Send()"';
					else if ($ver == 'x-priority')
						$err = 'can not set "X-Priority", for this, use function "Priority()"';
					else if ($ver == 'x-msmail-priority')
						$err = 'can not set "X-MSMail-Priority", for this, use function "Priority()"';
					else if ($ver == 'date')
						$err = 'can not set "Date", this value is automaticaly set';
					else if ($ver == 'content-type')
						$err = 'can not set "Content-Type", this value is automaticaly set';
					else if ($ver == 'content-transfer-encoding')
						$err = 'can not set "Content-Transfer-Encoding", this value is automaticaly set';
					else if ($ver == 'content-disposition')
						$err = 'can not set "Content-Disposition", this value is automaticaly set';
					else if ($ver == 'mime-version')
						$err = 'can not set "Mime-Version", this value is automaticaly set';
					else if ($ver == 'x-mailer')
						$err = 'can not set "X-Mailer", this value is automaticaly set';
					if ($err)
	...
	public function delheader($name = null)
	{
		try {
			if ($name == null) {
				...
				throw new Exception('invalid name value', 1);
		} catch (Exception $e) {
			return RSLMAIL_FUNC::exception_handler($e, false);
		}
	}
		...
		try {
			if ($name == null) {
				...
				if ($name != '' && ($name == 'localhost' || RSLMAIL_FUNC::is_hostname($name))) {
					...
					$this->_host = $name;
					if ($name != 'localhost')
						$mx = RSLMAIL_FUNC::is_win() ? RSLMAIL_FUNC::getmxrr_win($name, $mxarr) : getmxrr($name, $mxarr);
					return true;
				} else
					throw new Exception('invalid hostname value', 0);
			} else
				throw new Exception('invalid hostname type', 0);
		} catch (Exception $e) {
			return RSLMAIL_FUNC::exception_handler($e, false);
		}
	}
	...
	public function priority($level = null)
	{
		try {
			if ($level == null) {
				...
				if ($level == 1)
					$this->_priority = array('1', 'High');
				else if ($level == 3)
					$this->_priority = array('3', 'Normal');
				else if ($level == 5)
					$this->_priority = array('5', 'Low');
				...
				if ($level == 'high')
					...
					throw new Exception('invalid level value', 0);
			} else
				throw new Exception('invalid level type', 0);
			return true;
		} catch (Exception $e) {
			return RSLMAIL_FUNC::exception_handler($e, false);
		}
	...
	{
		if ($charset == null)
			$charset = RSLMAIL_MIME::MCHARSET;
		if ($encoding == null)
			$encoding = RSLMAIL_MIME::MENCODING;
		if ($disposition == null)
			$disposition = 'inline';
		if ($this->_text = RSLMAIL_MIME::message($value, 'text/plain', null, $charset, $encoding, $disposition, null, new RSLMAIL_Exception()))
			return true;
		else
			return false;
	}
	...
	public function html($value = null, $charset = null, $encoding = null, $disposition = null)
	{
		...
		if ($disposition == null)
			$disposition = 'inline';
		if ($this->_html = RSLMAIL_MIME::message($value, 'text/html', null, $charset, $encoding, $disposition, null, new RSLMAIL_Exception()))
			return true;
		else
			return false;
	}
	...
	public function attachfile($file = null, $type = null, $name = null, $charset = null, $encoding = null, $disposition = null, $id = null)
	{
		try {
			$error = '';
			if (!is_string($file))
				$error = 'invalid filename type';
			else {
				$file = RSLMAIL_FUNC::str_clear($file);
				$file = trim($file);
				if ($file != '' && is_file($file) && is_readable($file)) {
					if ($type == null)
						$type = RSLMAIL_FUNC::mimetype($file);
					if ($name == null) {
						$exp1 = explode('/', $file);
	...
	public function delattach($name = null)
	{
		try {
			if ($name == null) {
				...
				throw new Exception('invalid name type', 0);
		} catch (Exception $e) {
			return RSLMAIL_FUNC::exception_handler($e, false);
		}
	}
	...
	public function send($subject = null, $charset = null, $encoding = null)
	{
		...
		try {
			...
			if (count($errors) == 0) {
				...
				if ($this->_from)
					$hfrom = $this->_from['name'] ? $this->_from['name'] . ' <' . $this->_from['address'] . '>' : $this->_from['address'];
				else {
					$hfrom = @ini_get('sendmail_from');
					if ($hfrom == '' || !RSLMAIL_FUNC::is_mail($hfrom))
						$hfrom = (isset($_SERVER['SERVER_ADMIN']) && RSLMAIL_FUNC::is_mail($_SERVER['SERVER_ADMIN'])) ? $_SERVER['SERVER_ADMIN'] : 'postmaster@localhost';
				}
				$ato = $alladdrs = array();
				...
				foreach ($hlocal as $arrloc)
					$header['local'] .= $arrloc['name'] . ': ' . $arrloc['value'] . RSLMAIL_MIME::CRLF; foreach ($hclient as $arrcli)
					$header['client'] .= $arrcli['name'] . ': ' . $arrcli['value'] . RSLMAIL_MIME::CRLF;
