...
function lmsgetcookie($cookiename)
{
	...
	if (isset($_COOKIE[$Encodestr])) {
		...
		// Check if the cookie with appended suffix exists
		if (isset($_COOKIE[$resultstr])) {
			$cookiemd5 = $_COOKIE[$resultstr];

			// Compare the encoded value with the fetched cookie
			if ($Encodestring == $cookiemd5) {
				$decodevalue = decodevalue($stringret);
				$string1 = substr($decodevalue, 7);
				$resultstring = substr($string1, 0, -7);
				...
				return false; // If the comparison fails
			}
		} else {
			return false; // If the cookie with appended suffix doesn't exist
		}
	} else {
		return false; // If the main cookie doesn't exist
	}
}
