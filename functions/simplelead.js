...
{
	var passdata ="&submittype=getcontactnumber&dummy=" + Math.floor(Math.random()*100032680100);
		...
		success: function(response,status)
		{
			...
			{	
				...
				if(response1[0] == '1')
				{
					...
					$("#gridprocess").html('<font color="#FFFFFF"><strong>&nbsp; List of Leads => [Leads of Last 2 Days (' + response1[3] +' Records)] </strong></font>');
				}
...
function filtering(command)
{
	...
	else
	{
		...
		var passdata = "&submittype=filter&fromdate=" + encodeURIComponent($("#DPC_fromdate").val()) + "&todate=" + encodeURIComponent($("#DPC_todate").val()) + "&dealerid=" + encodeURIComponent($("#dealerid").val()) + "&givenby=" + encodeURIComponent($("#givenby").val()) + "&productid=" + encodeURIComponent($("#productid").val()) + "&leadstatus=" + encodeURIComponent($("#leadstatus").val()) + "&leadsubstatus=" + encodeURIComponent($("#leadsubstatus").val()) + "&filter_followupdate1=" + encodeURIComponent(filter_followupdate1) + "&filter_followupdate2=" + encodeURIComponent(filter_followupdate2) + "&dropterminatedstatus=" + encodeURIComponent(dropterminatedstatus)+"&searchtext="+ encodeURIComponent($("#srchhiddenfield").val())+"&subselection="+encodeURIComponent($("#subselhiddenfield").val())+"&datatype="+encodeURIComponent($("#datatypehiddenfield").val())+"&followedby="+encodeURIComponent($("#followedbyhidden").val())+"&leadsource="+encodeURIComponent($("#hiddensource").val())+"&grouplabel="+grouplabel+"&followupcheck="+followupcheck+"&remarks="+remarks.val()+"&dummy=" + Math.floor(Math.random()*10230000000) ; //alert(passdata);
		var queryString = "../ajax/simplelead.php";
		ajaxobjext56 = $.ajax(
		{
			type: "POST",url: queryString, data: passdata, cache: false,
...
function gridtoform(id)
{
	...
	if($('#hiddentype').val() == 'Admin' || $('#hiddentype').val() == 'Sub Admin' || $('#hiddentype').val() == 'Reporting Authority')
	{
		$('#selectlist').hide();
	}
			...
			{	
				...
				if(response3[0] == '1')
				{
					...
					if(response3[21] == 'Not Interested')
					{
						...
						if(response3[36] == 'Requirement does not match' || response3[36] == 'Other')
						{
							$('#leadMeetRemarks').show();
							$('#leadMeetRemarks').val(response3[35]);
						}
...
function mydemovalue()
{
    ...
    if(($('#tabgroupgridc1_2 #gridtable1 td:contains(Demo Given)').length) && ($('#form_leadstatus').val() == 'Demo Given'))
    {
        alert('Demo Given Status Can Not be Repeated')
    }
    else if(($('#tabgroupgridc1_2 #gridtable1 td:contains(Quote Sent)').length) && ($('#form_leadstatus').val() == 'Quote Sent'))
    {
        alert('Quote Sent Status Can Not be Repeated')
    }
    else if(($('#tabgroupgridc1_2 #gridtable1 td:contains(Perusing to Purchase)').length) && ($('#form_leadstatus').val() == 'Perusing to Purchase'))
    {
        alert('Perusing to Purchase Status Can Not be Repeated')
    }
    else if(($('#tabgroupgridc1_2 #gridtable1 td:contains(Order Closed)').length) && ($('#form_leadstatus').val() == 'Order Closed'))
    {
        alert('Order Closed Status Can Not be Repeated')
    }
    else if(($("#tabgroupgridc1_2 #gridtable1 tr:last").find("td:nth-child(2)").html()) == 'Not Interested' && ($('#tabgroupgridc1_2 #gridtable1 tr').length == '2'))
    {
...
function formsubmit(command)
{
	...
	if(contactperson.val() == '' && address.val() == '' && stdcode.val() == '' && phone.val() == '' && cell.val() == '' && emailid.val() == '')
	{
		msg_box.html(errormessage("Please Select a Lead First"));
		return false;
	}
	if (!form_recid.val())
	{ msg_box.html(errormessage("Please Select a Lead to Update.")); return false;}
	if (!form_leadstatus.val())
	{ msg_box.html(errormessage("Please Select the current status of the Lead.")); form_leadstatus.focus(); return false;}
	if (form_leadstatus.val() == 'Requirement Does not Meet')
	{  
		if(!form_leadMeetRemarks.val())
		{
			msg_box.html(errormessage("Please enter remarks.")); form_leadMeetRemarks.focus(); return false;
		}
	}
	...
	{  
		...
		{
			msg_box.html(errormessage("Please Select status.")); form_leadMeetRemarks.focus(); return false;
		}

		if (form_subleadstatus.val() == 'Requirement does not match' || form_subleadstatus.val() == 'Other')
		{
			if(!form_leadMeetRemarks.val())
			{
				msg_box.html(errormessage("Please enter remarks.")); form_leadMeetRemarks.focus(); return false;
	...
	if(confirmation)
	{
		...
		var passdata = "&submittype=save&form_recid=" + form_recid.val() + "&form_leadstatus=" + encodeURIComponent(form_leadstatus.val())+ "&form_subleadstatus=" + encodeURIComponent(form_subleadstatus.val())+ "&form_leadMeetRemarks=" + encodeURIComponent(form_leadMeetRemarks.val())+"&dummy=" + Math.floor(Math.random()*10230000000);//alert(passdata)
		//alert(passdata);
		var queryString = "../ajax/simplelead.php";
		ajaxobjext60 = $.ajax(
		{
...
function addfollowup()
{
	...
	if (!form_recid.val())
	{ followupmessage.html(errormessage("Please Select a Lead.")); return false;}
	if (!form_leadremarks.val())
	{ followupmessage.html(errormessage("Please Enter the remarks.")); form_leadremarks.focus(); return false;}
	...
	if (followupdate.val())
	{
		...
		{ followupmessage.html(errormessage("Enter a valid 'Follow-up Date' [dd-mm-yyyy].")); followupdate.focus(); return false;}
	}
	
	followupmessage.html(processing());
	var passdata = "&submittype=addfollowup&form_recid=" + form_recid.val() + "&form_leadremarks=" + encodeURIComponent(form_leadremarks.val()) + "&followupdate=" + encodeURIComponent($("#DPC_followupdate").val())+"&dummy=" + Math.floor(Math.random()*10230000000);
...
function showfollowups(leadid)
{
	//var form_recid = $("#form_recid");
	$("#followupmessage").html(processing());
	var passdata = "&submittype=showfollowups&form_recid=" + leadid +"&dummy=" + Math.floor(Math.random()*100032680100);
	var queryString = "../ajax/simplelead.php";
	ajaxobjext62 = $.ajax(
	{
		type: "POST",url: queryString, data: passdata, cache: false,
...
{
	if($('#addfollowup'))
	$('#addfollowup').attr('disabled',false);
}

function disableadd()
{
	if($('#addfollowup'))
	$('#addfollowup').attr('disabled',true);
}
...
function getmorerecords(startlimit,slnocount,showtype,type)
{
			...
			success: function(response,status)
			{
				...
				{	
					...
					if(ajaxresponse[0] == '1')
					{
						...
						$("#gridprocess").html(' <font color="#FFFFFF"><strong>&nbsp; List of Leads => [Leads of Last 2 Days (' + ajaxresponse[3] +' Records)] </strong></font>');
	...
	else if(type == 'filter')
	{
		...
		var passdata = "&submittype=filter&startlimit="+startlimit +"&slnocount="+slnocount+"&showtype="+showtype+"&fromdate=" + encodeURIComponent($("#hiddenfromdate").val()) + "&todate=" + encodeURIComponent($("#hiddentodate").val()) + "&dealerid=" + encodeURIComponent($("#hiddendealerid").val()) + "&givenby=" + encodeURIComponent($("#hiddengivenby1").val()) + "&productid=" + encodeURIComponent($("#hiddenproductid").val()) + "&leadstatus=" + encodeURIComponent($("#hiddenleadstatus").val()) + "&leadsubstatus=" + encodeURIComponent($("#hiddenleadsubstatus").val()) + "&filter_followupdate1=" + encodeURIComponent($("#filter_followupdate1hdn").val()) + "&filter_followupdate2=" + encodeURIComponent($("#filter_followupdate2hdn").val()) + "&dropterminatedstatus=" + encodeURIComponent(dropterminatedstatus)+"&searchtext="+ encodeURIComponent($("#srchhiddenfield").val())+"&subselection="+encodeURIComponent($("#subselhiddenfield").val())+"&datatype="+encodeURIComponent($("#datatypehiddenfield").val())+"&followedby="+encodeURIComponent($("#followedbyhidden").val())+"&leadsource="+encodeURIComponent($("#hiddensource").val())+"&grouplabel="+encodeURIComponent($("#hiddengrouplabel").val())+"&followupcheck="+followupcheck+"&remarks="+remarks.val()+"&dummy=" + Math.floor(Math.random()*10230000000);
...
function updatedata()
{
	...
	if(contactperson.val() == '' && address.val() == '' && stdcode.val() == '' && phone.val() == '' && cell.val() == '' && emailid.val() == '' && $("#id").html() == '')
	{
	...
	else
	{
		...
		else
		{
			...
			$('#msg_box').html('');
			var id = $('#hiddenid').val(); 
			var passdata = "&submittype=updatedata&id="+encodeURIComponent(id)+"&contactperson="+contactperson.val()+"&address="+address.val()+"&stdcode="+stdcode.val()+"&phone="+phone.val()+"&cell="+cell.val()+"&emailid="+emailid.val()+"&dummy=" + Math.floor(Math.random()*10230000000); //alert(passdata);
			var queryString = "../ajax/simplelead.php";
			ajaxobjext59 = $.ajax(
			{
				type: "POST",url: queryString, data: passdata, cache: false,
...
{
	$('#followuptotal').html(processing()+'  ' + '<span onclick = "abortfollowupajaxprocess(\'initial\')" class="abort">(STOP)</span>');
	var passdata = "&submittype=followupforday&startlimit="+startlimit+"&dummy=" + Math.floor(Math.random()*10230000000); //alert(passdata)
	var queryString = "../ajax/simplelead.php";
	ajaxobjext63 = $.ajax(
	{
		type: "POST",url: queryString, data: passdata, cache: false,
		success: function(response,status)
		{
			...
			{	
				...
				if(ajaxresponse[0] == '1')
				{
					...
					$('#followuptotal').html('<font color="#FFFFFF"><strong>&nbsp;Total Count :  ' + ajaxresponse[3] +'</strong></font>');
...
{
	$('#followuptotal').html(processing()+'  ' + '<span onclick = "abortfollowupajaxprocess(\'showmore\')" class="abort">(STOP)</span>');
	var passdata = "&submittype=followupforday&startlimit="+startlimit +"&slnocount="+slnocount+"&showtype="+showtype+"&dummy=" + Math.floor(Math.random()*10230000000); //alert(passdata)
	var queryString = "../ajax/simplelead.php";
	ajaxobjext64 = $.ajax(
	{
		type: "POST",url: queryString, data: passdata, cache: false,
		success: function(response,status)
		{
			...
			{	
				...
				if(ajaxresponse[0] == '1')
				{
					...
					$('#tabgroupgridfup1_1').html($('#resultgridfup').html().replace(/\<\/table\>/gi,'')+ ajaxresponse[1]) ;
					$('#getmorelinkfup').html(ajaxresponse[2]);
					$('#followuptotal').html('<font color="#FFFFFF"><strong>&nbsp;Total Count :  ' + ajaxresponse[3] + '</strong></font>');
...
{
	$('#followuptotaln').html(processing() +'  '+ '<span onclick = "abortnofollowupajaxprocess(\'initial\')" class="abort">(STOP)</span>' );
	var passdata = "&submittype=nofollowup&startlimit="+startlimit+"&dummy=" + Math.floor(Math.random()*10230000000); //alert(passdata)
	var queryString = "../ajax/simplelead.php";
	ajaxobjext65 = $.ajax(
	{
		type: "POST",url: queryString, data: passdata, cache: false,
		success: function(response,status)
		{
			...
			{	
				...
				if(ajaxresponse[0] == '1')
				{
					...
					$('#followuptotaln').html('<font color="#FFFFFF"><strong>&nbsp;Total Count :  ' + ajaxresponse[3] +'</strong></font>') ;
				}
				else
				{
...
{
	$('#followuptotaln').html(processing()+'  '+ '<span onclick = "abortnofollowupajaxprocess(\'showmore\')" class="abort">(STOP)</span>');
	var passdata = "&submittype=nofollowup&startlimit="+startlimit +"&slnocount="+slnocount+"&showtype="+showtype+"&dummy=" + Math.floor(Math.random()*10230000000); //alert(passdata)
	var queryString = "../ajax/simplelead.php";
	ajaxobjext66 = $.ajax(
	{
		type: "POST",url: queryString, data: passdata, cache: false,
		success: function(response,status)
		{
			...
			{	
				...
				if(ajaxresponse[0] == '1')
				{
					...
					$('#tabgroupgridn1_1').html($('#resultgridn1').html().replace(/\<\/table\>/gi,'')+ ajaxresponse[1]);
					$('#getmorelinkn1').html(ajaxresponse[2]);
					$('#followuptotaln').html('<font color="#FFFFFF"><strong>&nbsp;Total Count :  ' + ajaxresponse[3] +'</strong></font>') ;
				}
				else
...
{
	$('#gridprocessnv').html(processing()+'  '+ '<span onclick = "abortnotviewedajaxprocess(\'initial\')" class="abort">(STOP)</span>');
	var passdata = "&submittype=notviewed&startlimit="+startlimit+"&dummy=" + Math.floor(Math.random()*10230000000); //alert(passdata)
	var queryString = "../ajax/simplelead.php"; //alert(passdata);
	ajaxobjext67 = $.ajax(
	{
		type: "POST",url: queryString, data: passdata, cache: false,
		success: function(response,status)
		{
			...
			{	
				...
				if(ajaxresponse[0] == '1')
				{
					...
					$('#gridprocessnv').html('<font color="#FFFFFF"><strong>&nbsp;Total Count :  ' + ajaxresponse[3] +'</strong></font>');
...
{
	$('#gridprocessnv').html(processing()+'  '+ '<span onclick = "abortnotviewedajaxprocess(\'showmore\')" class="abort">(STOP)</span>');
	var passdata = "&submittype=notviewed&startlimit="+startlimit +"&slnocount="+slnocount+"&showtype="+showtype+"&dummy=" + Math.floor(Math.random()*10230000000); //alert(passdata)
	var queryString = "../ajax/simplelead.php";
	ajaxobjext68 = $.ajax(
	{
		type: "POST",url: queryString, data: passdata, cache: false,
		success: function(response,status)
		{
			...
			{	
				...
				if(ajaxresponse[0] == '1')
				{
					...
					$('#tabgroupgridnv1_1').html( $('#resultgridnv1').html().replace(/\<\/table\>/gi,'')+ ajaxresponse[1]) ;
					$('#getmorelinknv1').html(ajaxresponse[2]);
					$('#gridprocessnv').html('<font color="#FFFFFF"><strong>&nbsp;Total Count :  ' + ajaxresponse[3] + '</strong></font>');
				}
				else
...
function viewotherleads(startlimit)
{
	...
	else
	{
		...
		var passdata = "&submittype=otherleads&startlimit="+startlimit+"&id="+id+"&dummy=" + Math.floor(Math.random()*10230000000); 
		var queryString = "../ajax/simplelead.php"; //alert(passdata);
		ajaxobjext69 = $.ajax(
		{
			type: "POST",url: queryString, data: passdata, cache: false,
...
function getmoreofotherleads(startlimit,slnocount,showtype)
{
	...
	var id = $('#hiddenid').val();
	var passdata = "&submittype=otherleads&startlimit="+startlimit +"&slnocount="+slnocount+"&showtype="+showtype+"&id="+id+"&dummy=" + Math.floor(Math.random()*10230000000); //alert(passdata)
	var queryString = "../ajax/simplelead.php";
	ajaxobjext70 = $.ajax(
	{
		type: "POST",url: queryString, data: passdata, cache: false,
		success: function(response,status)
		{
			...
			{	
				...
				if(ajaxresponse[0] == '1')
				{
					...
					$('#tabgroupgridc1_4').html($('#resultgrid4').html().replace(/\<\/table\>/gi,'')+ ajaxresponse[1]);
					$('#getmorelink4').html(ajaxresponse[2]);					
				}
				else
				{
...
function downloadlogs(startlimit)
{
	...
	else
	{
		...
		var passdata = "&submittype=downloadlogs&startlimit="+startlimit+"&id="+id+"&dummy=" + Math.floor(Math.random()*10230000000); //alert(passdata)
		var queryString = "../ajax/simplelead.php"; //alert(passdata);
		ajaxobjext71 = $.ajax(
		{
			type: "POST",url: queryString, data: passdata, cache: false,
...
function getmoreofdownloadlogs(startlimit,slnocount,showtype)
{
	...
	var id = $('#hiddenid').val();
	var passdata = "&submittype=downloadlogs&startlimit="+startlimit +"&slnocount="+slnocount+"&showtype="+showtype+"&id="+id+"&dummy=" + Math.floor(Math.random()*10230000000); //alert(passdata)
	var queryString = "../ajax/simplelead.php";
	ajaxobjext72 = $.ajax(
	{
		type: "POST",url: queryString, data: passdata, cache: false,
		...
		{	
			...
			else
			{
				...
				if(ajaxresponse[0] == '1')
				{
					...
					$('#tabgroupgridc1_5').html($('#resultgrid5').html().replace(/\<\/table\>/gi,'')+ ajaxresponse[1]);
					$('#getmorelink5').html(ajaxresponse[2]);					
				}
				else
				{
...
function resetlink(type)
{
	if(type == 'open')
	{
		...
		var val = "Are you sure you want to Transfer this Lead to " +selected_text +" ?";
		var confirmation = confirm(val);
		if(confirmation)
		{
			...
			var id = $('#hiddenid').val(); 
			var passdata = "&submittype=transferlead&id="+encodeURIComponent(id)+"&todealerid="+todealerid+"&dummy=" + Math.floor(Math.random()*10230000000); //alert(passdata)
			var queryString = "../ajax/simplelead.php";
			ajaxobjext73 = $.ajax(
			{
				type: "POST",url: queryString, data: passdata, cache: false,
...
function leadtrack(type)
{
	...
	else if(type == 'sendsms')
	{
		...
		{
			$('#prior-sms-error').html(errormessage('Please Save Cell Number and then proceed.'));
			leadcardcell.focus();
			return false;
		}
	}
...
function createlead()
{
	...
	error.html(processing());
	var passdata = "&submittype=createlead&leadid="+leadid+"&productid="+productid.val()+"&remarks="+remarks.val()+"&radiovalue="+radiovalue+"&dealer="+dealer.val()+"&newleadcontactperson="+newleadcontactperson.val()+"&newleadcell="+newleadcell.val()+"&newleademailid="+newleademailid.val()+"&newleadsource="+newleadsource.val()+"&dummy=" + Math.floor(Math.random()*10230000000); //alert(passdata)
	var queryString = "../ajax/simplelead.php";
	ajaxobjext75 = $.ajax(
	{
		type: "POST",url: queryString, data: passdata, cache: false,
...
function sendsms(actiontype)
{
	if(actiontype == 'sms')
	{
		...
		{
			error.html(processing());
			var passdata = "&submittype=sendsms&cellnumber="+cellnumber.html()+"&smstext="+smstext.val()+"&leadid="+leadid.val()+"&dummy=" + Math.floor(Math.random()*10230000000); //alert(passdata)
			var queryString = "../ajax/simplelead.php";	
			ajaxobjext76 = $.ajax(
			{
				type: "POST",url: queryString, data: passdata, cache: false,
...
{
	var leadid = $('#hiddenid');
	var passdata = "&submittype=showsmslogs&leadid="+leadid.val()+"&startlimit="+startlimit+"&dummy=" + Math.floor(Math.random()*10230000000); //alert(passdata)
	var queryString = "../ajax/simplelead.php";
	ajaxobjext77 = $.ajax(
	{
		type: "POST",url: queryString, data: passdata, cache: false,
...
function getmoreofsmslogs(startlimit,slnocount,showtype)
{
	...
	var id = $('#hiddenid').val();
	var passdata = "&submittype=showsmslogs&startlimit="+startlimit +"&slnocount="+slnocount+"&showtype="+showtype+"&leadid="+id+"&dummy=" + Math.floor(Math.random()*10230000000); //alert(passdata)
	var queryString = "../ajax/simplelead.php";
	ajaxobjext78 = $.ajax(
	{
		type: "POST",url: queryString, data: passdata, cache: false,
		success: function(response,status)
		{
			...
			{	
				...
				if(ajaxresponse[0] == '1')
				{
					...
					$('#sendsmsc1_1').html($('#resultgridsms').html().replace(/\<\/table\>/gi,'')+ ajaxresponse[1]);
					$('#getmoresmslink').html(ajaxresponse[2]);					
				}
				else
				{
...
{
	$('#sms-error').html('');
	var passdata = "&submittype=gridtoformsms&smsid="+smsid+"&dummy=" + Math.floor(Math.random()*10230000000); //alert(passdata)
	var queryString = "../ajax/simplelead.php";
	ajaxobjext79 = $.ajax(
	{
		type: "POST",url: queryString, data: passdata, cache: false,
...
function closeopentxtbox(type)
{
	if(type == 'open')
	{
		...
		var confirmation = "Are you sure to update the Initial Remarks as " + initialremarks + "??";
		var val = confirm(confirmation);
		if(val)
		{
			var passdata = "&submittype=saveinitialremarks&form_recid=" + $("#form_recid").val() + "&form_remarks=" +initialremarks +"&dummy=" + Math.floor(Math.random()*10230000000);//alert(passdata)
			var queryString = "../ajax/simplelead.php";
			ajaxobjext101 = $.ajax(
			{
				type: "POST",url: queryString, data: passdata, cache: false,
...
{
    var leadid = value;
    var passdata = "&submittype=checkforleadid&leadid=" + leadid +"&dummy=" + Math.floor(Math.random()*10230000000);//alert(passdata)
			var queryString = "../ajax/simplelead.php";
			ajaxobjext101 = $.ajax(
			{
				type: "POST",url: queryString, data: passdata, cache: false,
				success: function(response,status)
				{
					...
					{	
						...
						if(split[0] == '1')
						{
    						...
    						{
    						    alert(split[i]);
    						    $('#form_leadstatus option[value="'+split[i]+'"]').attr('disabled', 'disabled');
    						}
						...
						if(split[0] == '2')
						{
    						...
    						{
    						    alert(split[i]);
    						    $('#form_leadstatus option[value="'+split[i]+'"]').removeAttr('disabled');
    						}
